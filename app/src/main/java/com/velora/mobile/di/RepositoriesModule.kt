...
object RepositoriesModule {
    ...
    ): AuthRepository = FirebaseAuthRepositoryImpl(auth, db)

    @Provides
    @Singleton
    fun jobsRepo(db: FirebaseFirestore): JobsRepository = FirestoreJobsRepositoryImpl(db)

    @Provides
    @Singleton
    fun resumeAnalyzer(): ResumeAnalyzer = LocalResumeAnalyzer()
}